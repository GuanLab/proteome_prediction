...
# training data
bt=as.matrix(read.delim(paste0(path0,"breast_proteome.txt"),check.names=F,row.names=1))
ot=as.matrix(read.delim(paste0(path2,"train_ova_proteome_",i,".txt"),check.names=F,row.names=1))
ind_target=intersect(rownames(ot),rownames(bt)) # common target gene
train=cbind(ot[ind_target,],bt[ind_target,])

# test
test_all=as.matrix(read.delim(paste0(path2,"test_ova_proteome_",i,".txt"),check.names=F,row.names=1))
