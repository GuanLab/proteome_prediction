...
for (j in args){
	...
	for(i in ind){
	    tmp=is.na(output[i,])
	    output[i,tmp]=min(output[i,!tmp],na.rm=T)
	}
	sample_min=apply(output,2,min,na.rm=T)
	ind = apply(output,1,function(x){all(is.na(x))}) # completely missing rows
	output[ind,]=rep(sample_min,each=sum(ind))

	output=cbind(rownames(output),output);colnames(output)[1]="Gene_ID"
	write.table(output,file=paste0(j,".min_imputation"),quote=F,sep="\t",row.names=F)
}
